...
const ServerCard = ({
  ...
  return (
    <div data-aos="zoom-in">
      <Link to={`/Vacancy/${id}`} className={`server-card1 ${clasCard}`}>
        <div className="card-img-block">
          ...
          <img src={icon ? icon : Images} alt="images" className={`img-icon ${classes}`} />
        </div>
        <h6 className="card-name">{name}</h6>
        <div className="card-info">{ReactHtmlParser(info)}</div>
        <div className="card_extra_title_blocks">{ReactHtmlParser(extraTitle)}</div>
          <div
            to="/singleVacancy"
            className={`home-page2-link vacancy_singlie_link ${cardLink}`}
          >
            {" "}
            {lang === "en"
              ? "More"
